...
if ( ! function_exists( 'wcanvas_boilerplate_posted_on' ) ) :
	/**
	 * Prints HTML with meta information for the current post-date/time.
	 */
	function wcanvas_boilerplate_posted_on() {
		...
		$posted_on = sprintf(
			...
			esc_html_x( 'Posted on %s', 'post date', 'wcanvas-boilerplate' ),
...
if ( ! function_exists( 'wcanvas_boilerplate_posted_by' ) ) :
	/**
	 * Prints HTML with meta information for the current author.
	 */
	function wcanvas_boilerplate_posted_by() {
		$byline = sprintf(
			/* translators: %s: post author. */
			esc_html_x( 'by %s', 'post author', 'wcanvas-boilerplate' ),
...
if ( ! function_exists( 'wcanvas_boilerplate_entry_footer' ) ) :
	/**
	 * Prints HTML with meta information for the categories, tags and comments.
	 */
	function wcanvas_boilerplate_entry_footer() {
		// Hide category and tag text for pages.
		if ( 'post' === get_post_type() ) {
			/* translators: used between list items, there is a space after the comma */
			$categories_list = get_the_category_list( esc_html__( ', ', 'wcanvas-boilerplate' ) );
			if ( $categories_list ) {
				/* translators: 1: list of categories. */
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'wcanvas-boilerplate' ) . '</span>', $categories_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}

			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'wcanvas-boilerplate' ) );
			if ( $tags_list ) {
				/* translators: 1: list of tags. */
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'wcanvas-boilerplate' ) . '</span>', $tags_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
		}
			...
			comments_popup_link(
				sprintf(
					wp_kses(
						...
						__( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'wcanvas-boilerplate' ),
						array(
							'span' => array(
								'class' => array(),
							),
...
if ( ! function_exists( 'wcanvas_boilerplate_post_thumbnail' ) ) :
	/**
	 ...
	 */
	function wcanvas_boilerplate_post_thumbnail() {
...
if( !function_exists( 'wcanvas_boilerplate_heading' ) ){
	 ...
	 */
	function wcanvas_boilerplate_heading( $field_name = 'heading', $params = [] ){
		$params += [
...
if( !function_exists( 'wcanvas_boilerplate_button_group' ) ){

	/**
	 * Output standard button group support group contents
	 * Can be used for sub fields or top level by passing the appropriate function param
	 */
	function wcanvas_boilerplate_button_group( $field_name = 'button_section', $params = [] ){
...
if( !function_exists( 'wcanvas_boilerplate_needs_rel_tags' ) ){

	/**
	 * Determine if target url matches site domain for adding of noopener noreferrer tags in templates
	 */
	function wcanvas_boilerplate_needs_rel_tags( $url ){
